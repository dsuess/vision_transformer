...
class SentencepieceTokenizer:
    ...
    def preprocess_tf(self, text):
        ...
        return tf.pad(
            tokens,
            [(0, self.max_len - tf.shape(tokens)[0])],
...
class PreprocessImages:
    ...
    def _resize_small(self, image):  # pylint: disable=missing-docstring
        ...
        ratio = tf.cast(self.size, tf.float32) / tf.cast(tf.minimum(h, w), tf.float32)
    ...
    def _resize(self, image):
        return tf.image.resize(image, size=[self.size, self.size], method="bilinear")
    ...
    def __call__(self, images):
        ...
        return np.stack([self.preprocess_tf(tf.constant(image)) for image in images])
